...
function createDb (dbName, host, port) {
...
module.exports = {
  ...
  removeRecords: function (config, record) {
    var connect = nano(config.dbUrl),
        db = connect.use(config.dbName);

    db.destroy(record.id, record.rev, function (err, body) {
      if (!err) console.log(body);
    })
  },
  removeByIds: function (config, callback) {
    var connect = nano(config.dbUrl),
        db = connect.use(config.dbName),
        ids = [];
  ...
  makeNewIds: function (config, callback) {
    var connect = nano(config.dbUrl),
        db = connect.use(config.dbName),
        ids = [];
